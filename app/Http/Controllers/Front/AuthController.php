...
class AuthController extends Controller {
    ...
    public function register() {
        ...
        if (empty(Input::get('first_name'))) {
            ...
            exit;
        } elseif (empty(Input::get('gender'))) {
            echo json_encode(['message' => 'Please select gender.']);
            exit;
            ...
            if (!empty($userData)) {
                ...
                if (!empty($password)) {
                    if ($this->authenticate($userData['0']['email'], Input::get('password'), $userData['0']['email'])) {
                       ...
                       /*$cart_details = Cart::where('user_id', $userData['0']['id'])->get();
                       if(!empty($cart_details)){
                       foreach ($cart_details as $cart_detail){
                          Wishlist::where('user_id', $userData['0']['id'])
                                  ->where('product_id', $cart_detail->product_id)
                                  ->where('color_id', $cart_detail->color_id)
                                  ->delete(); 
                       }
                       }*/
