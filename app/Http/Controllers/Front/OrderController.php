    ...
    public function paymentCallback()
    {
        ...
        $carts = Cart::where('user_id', $auth->id)->delete();
          ...
          $orders = Order::where('order_no', $transaction->response()['ORDERID'])->first();
            ...
            $this->invoice($orders->first_name,$orders->last_name,$orders->email,$orders->shipping_amount,$prorow,$orders->order_no,$txnmsg,$orders->item_total_amount,$orders->address_1,$orders->address_2,$orders->pincode,$city->city_name,$state->state_name,'paytm',$orders->discount);
         ...
         if($pay_data->responce_msg == 'Txn Success'){
            ...
            $msg1 = str_replace('Customer', 'You', $pay_data->responce_msg).'  Your Order Id is:- '.$pay_data->order_id; 
         }
         $str = $msg1.' <br />  Your Transaction Id is:- '.$pay_data->txn_id;
            
         //$str = $pay_data->responce_msg.'  Your Order Id is:- '.$pay_data->order_id.' And  Your Transaction Id is:- '.$pay_data->txn_id;
...
//pr($body);exit;
    ...
    public function order(Request $request)
    {
        ...
        $carts = Cart::where('user_id', $auth->id)->delete();
            ...
            $this->invoice($orders->first_name,$orders->last_name,$orders->email,$orders->shipping_amount,$prorow,$orders->order_no,$orders->pay_mode,$orders->item_total_amount,$orders->address_1,$orders->address_2,$orders->pincode,$city->city_name,$state->state_name,$orders->pay_mode,$orders->discount);
           return view('front.payments.success', compact('orders','orderDetails','state','city'));
        }else if($input['pay_mode'] == 'paytm'){ 
            Session::forget('ORDER_NO');
            $is_update = Order::where('order_no', $input['order_no'])
                    ->update(['status'=>1,'shipping_amount'=>'0.00','pay_mode'=>'paytm']);
            $orders = Order::where('order_no', $input['order_no'])->first();
        $payment = PaytmWallet::with('receive');
        $payment->prepare([
    ...
    { //pr($request);exit;
     ...
     public function paypal() {
      ...
      public function payusuccess(Request $request) {
        ...
        $carts = Cart::where('user_id', $auth->id)->delete();
         ...
         return view('front.payments.payu_success', compact('orderDetails','orders','state','city','pay_data'));
     }
     ...
     public function paypal_success() { 
        ...
        if($request){
             ...
             $orders = Order::where('order_no', $request['cm'])->first();
        ...
        $this->invoice($orders->first_name,$orders->last_name,$orders->email,$orders->shipping_amount,$prorow,$orders->order_no,$orders->pay_mode,$orders->item_total_amount,$orders->address_1,$orders->address_2,$orders->pincode,$city->city_name,$state->state_name,$orders->pay_mode,$orders->discount);
         ...
         return view('front.payments.paypal_success', compact('pay_data','orderDetails','orders','state','city'));
     }
