...
?>
                        ...
                        <div class="col-md-4 single-product-wrap men kid bagshoes">
                            <figure class="product-thumbnail">
                                ...
                                <img class="secondary-thumb" src="{{PRODUCT_IMAGE_URL.$latest_product->product_image[0]['image_name']}}" alt="Product"/>
                               <?php }else{?>
                                <img class="secondary-thumb" src="{{asset('img/no-image.png')}}" alt="Product"/>
                               <?php }?>
                               
                                <?php if(isset($latest_product->product_image[0]) && !empty($latest_product->product_image[0])) {?>
                                    <img class="primary-thumb" src="{{PRODUCT_IMAGE_URL.$latest_product->product_image[0]['image_name']}}" alt="Product"/>
                                <?php }else{?>
                                ...
                                <figcaption class="product-hvr-content">
                                    ...
                                    <a id="myBtn_view" attId="{{$key}}" class="view_btn_class" title="Quick view">
                                    ...
                                    <a name="wishlist_btn"  id="wishlist_btn_red" attrColor = "{{ $latest_product->product_image[0]['color_id'] }}" attrId="{{$latest_product->id}}" class="last_icn wishBtnManageColor wishlist_btn_red wishlist-btn-color">
                                    <i class="fa fa-heart" aria-hidden="true"></i>
                                    </a>
                                    @else
                                    <a name="wishlist_btn" id="wishlist_btn" attrColor = "{{ $latest_product->product_image[0]['color_id'] }}" attrId="{{$latest_product->id}}" title="Add to wishlist" class="last_icn wishlist_btn wishBtnManageColor">
                                    <i class="fa fa-heart" aria-hidden="true"></i>
                                    </a>
                                    @endif
                                    @else
						   ...
						   <div class="modal-content_view">
							  <div class="modal-body_view">
								 <div class="slideshow_modal">
									...
									@foreach($latest_product->product_image as $key1=>$val)
...
<div class="col-md-9 slideshow-container">
	...
	<?php
        if(!empty($featured_product->product_image[0])){
	...
	<a class="prev" onclick="plusSlides(-1)"><img src="{{asset('img/arrow.png')}}"></a>
	   ...
	   <div class="col-md-5 text_p">
		<span>FOR {{ucfirst($featured_product->product_category->cat_name)}}</span> 
                @if(!empty($featured_product->product_subcategory))
	...
	<div class="col-md-6">
	  <a href="{{URL::to('productDetail/'.$featured_product->slug.'/'.$colorLists->slug)}}"><img src="{{PRODUCT_IMAGE_URL.$featured_product->product_image[0]['image_name']}}" style="width: 100%;"></a>
	</div>
	</div>
	@endforeach
    ...
    <div class="container p-0">
        <div class="row no-gutters">
            <div class="col-lg-12">
			 ...
			 <?php $colorLists = CommonHelpers::getProductColor($random_product->product_image[0]['color_id']);?>		
                        <div class="col-md-4 single-product-wrap men kid bagshoes">
                            <figure class="product-thumbnail">
                                <a href="{{URL::to('productDetail/'.$random_product->slug.'/'.$colorLists->slug)}}" class="d-block">
                                        @if(isset($random_product->product_image[0]) && !empty($random_product->product_image[0]))
                                        <img class="secondary-thumb" src="{{PRODUCT_IMAGE_URL.$random_product->product_image[0]['image_name']}}" alt="Product"/>
                                        @else
                                        <img class="secondary-thumb" src="{{asset('img/no-image.png')}}" alt="Product"/>
                                    @endif
									
					@if(isset($random_product->product_image[0]) && !empty($random_product->product_image[0]))
                                    <img class="primary-thumb" src="{{PRODUCT_IMAGE_URL.$random_product->product_image[0]['image_name']}}" alt="Product"/>
                                    @else
                                    <img class="primary-thumb" src="{{asset('img/no-image.png')}}" alt="Product"/>
                                    @endif
                                ...
                                <figcaption class="product-hvr-content">
                                    ...
                                    @if(Auth::check())
                                     <?php $auth = LoginUser();
                                        $wishlist = CommonHelpers::wishlist_list($auth->id, $random_product->id, $random_product->product_image[0]['color_id']);
                                        ?>
                                    
                                    @if(isset($wishlist['0']['id']) && !empty($wishlist['0']['id']))
                                    <a name="wishlist_btn"  id="wishlist_btn_red" attrColor = "{{ $random_product->product_image[0]['color_id'] }}" attrId="{{$random_product->id}}" class="last_icn wishBtnManageColor wishlist_btn_red wishlist-btn-color">
                                    <i class="fa fa-heart" aria-hidden="true"></i>
                                    </a>
                                    @else
                                    <a name="wishlist_btn" id="wishlist_btn" attrColor = "{{ $random_product->product_image[0]['color_id'] }}" attrId="{{$random_product->id}}" title="Add to wishlist" class="last_icn wishlist_btn wishBtnManageColor">
                                    <i class="fa fa-heart" aria-hidden="true"></i>
                                    </a>
                                    @endif
                                    @else
                                    ...
                                    <div class="prod-btn-group">
                                        ...
                                        ?>
                                         <p class="prc">
                                             {!! CURRENCY !!}&nbsp;
                                             <b>{{$discount_price}}</b> 
                                              @if(isset($random_product->discount) && !empty($random_product->discount) && $random_product->discount != 0.00)
                                             <sub>{!! CURRENCY !!}&nbsp; 
                                                @if(CURRENCY == '<i class="fa fa-usd" aria-hidden="true"></i>')
						   ...
						   <div class="modal-content_view">
							  <div class="modal-body_view">
								 <div class="slideshow_modal2 margin-img">
									...
									@foreach($random_product->product_image as $key1=>$val)
