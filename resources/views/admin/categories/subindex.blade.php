...
<section class="content-header">
  ...
  {!!  Html::decode(Html::link(route('admin.addsubcategory', $slug),"<i class='fa  fa-plus'></i>",['class'=>'btn btn-primary','data-toggle'=>'tooltip','title'=>trans('Add')])) !!}				  
...
<!--                  <th  width="15%" align="center">{{trans('admin.PROFILE_IMAGE')}}</th>-->
           ...
           <tr>
              ...
              <td>
                        {!!  Html::decode(Html::link(route('admin.editsubcategory', [$category->slug, $slug]),"<i class='fa  fa-edit'></i>",['class'=>'btn btn-primary','data-toggle'=>'tooltip','title'=>trans('admin.EDIT')])) !!}				  
                           <?php /*?> {!! Form::model($category, ['url' => '/admin/categories/subindex/' . $category->id.'/'.$slug]) !!}
                            ...
                            {!! Form::close() !!}<?php */?>
                        {!!  Html::decode(Html::link(route('admin.subsubindex', $category->slug),"<i class='fa fa-th'></i>",['class'=>'btn btn-primary','data-toggle'=>'tooltip','title'=>'Sub Sub Category'])) !!}
              </td>
           </tr>
           <?php $i++; ?>
            @endforeach
